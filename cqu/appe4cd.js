...
;(function($, factory) {
	...
	factory['filter-children'] = function (elem) {
		...
		let searchInput = document.getElementById('filter-children-searchbox');

		let cards = elem.querySelectorAll('.your-account__children__filter-row')

		searchInput.addEventListener('keyup', (searchbox) => {
			clearTimeout(typingTimer);
			let search_query = searchbox.target.value.toLowerCase();
			typingTimer = setTimeout(liveSearch, typeInterval, cards, search_query);
		});

	};

	function liveSearch(cards, search_query) {
		// Loop through the cards
		for (var i = 0; i < cards.length; i++) {
			// If the text is within the card...
			if(cards[i].dataset.filterContent
				...
				if ($question.hasClass('quiz__question--actionable')) {
					...
					if (options.playable) {
						...
						if ($answer.data('iscorrect')) {
							...
							$status.addClass('correctly-answered').html('<span class="quiz__question__result__status--correct">Tebrikler!</span>')
							...
							$status.addClass('incorrectly-answered').html('<span class="quiz__question__result__status--incorrect">Olmadı.</span> Doğru cevap <span class="quiz__question__result__parrot">' + $correctAnswer.html() + '</span>');
